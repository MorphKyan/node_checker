...
export function filterNodes(nodes: NodeResult[], filters: NodeFilters): NodeResult[] {
  return nodes.filter((node) => {
    ...
    if (filters.minScore && node.total_score < Number(filters.minScore)) return false;
    if (filters.maxTtfb && node.probe.ttfb_ms > Number(filters.maxTtfb)) return false;
    if (filters.minSpeed && node.download_speed_mbps < Number(filters.minSpeed)) return false;
